...
@section('content')
    <div class="bg-white shadow-md rounded-lg p-6 mb-8">
        <h2 class="text-2xl font-semibold text-gray-800 mb-6">Kirim Peringatan</h2>

        <form method="POST" action="{{ route('warnings.store') }}" class="space-y-6">
            @csrf
            <div class="relative">
                    ...
                    @foreach ($wifiUsers as $user)
                        <div class="px-4 py-2">
                            <label class="flex items-center space-x-2">
                                <input type="checkbox" name="action_{{ $user->user_id }}" value="warning" data-user-id="{{ $user->user_id }}" class="rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500">
                                <span>Kirim Peringatan ({{ $user->user_id }})</span>
                            </label>
                        </div>
                    @endforeach
                </div>
            ...
            <div>
                <label for="message" class="block text-sm font-medium text-gray-700">Pesan Peringatan</label>
    ...
    <div class="bg-white shadow-md rounded-lg p-6">
        <h2 class="text-2xl font-semibold text-gray-800 mb-6">Daftar Peringatan Terkirim</h2>
        @if ($warnings->isEmpty())
            <p class="text-gray-600">Belum ada peringatan yang dikirim.</p>
        @else
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @foreach ($warnings as $warning)
                            <tr>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $warning->user_id }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $warning->wifiUser->wifi_package }}</td>
                                <td class="px-6 py-4 text-sm text-gray-900">{{ $warning->message }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $warning->sent_at->format('d/m/Y') }}</td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
